...
func ValidateRegister(ctx context.Context, u *models.User) *models.UserErrors {
	...
	if !valid.IsEmail(u.Email) {
		e.Err, e.Msg = true, "Must be a valid email"
	}
	...
	if !(len(u.Password) >= 8 && valid.HasLowerCase(u.Password) && valid.HasUpperCase(u.Password) && re.MatchString(u.Password)) {
		e.Err, e.Msg = true, "Length of password should be atleast 8 and it must be a combination of uppercase letters, lowercase letters and numbers"
	}
